...
//=================>Timer 0<=======================
const uint16_t TIMER_PRESCALER_0 = 50;  // CPU APB CLock Frequency*(1/prescaler)
const uint16_t TIMER_TICK_0 = 3125;    // TimerTick = CPU_CLK_FREQ / (TIMER_PRESCALER * ADC_FREQ) Hard coded because u_16_integer size limit
hw_timer_t* Timer0_Cfg = NULL;
// portMUX_TYPE timerMux = portMUX_INITIALIZER_UNLOCKED;

//=================>Timer 1<=======================
const uint16_t TIMER_PRESCALER_1 = 50;  // CPU APB CLock Frequency*(1/prescaler)
const uint16_t TIMER_TICK_1 = 6250;    // TimerTick = CPU_CLK_FREQ / (TIMER_PRESCALER * ADC_FREQ) Hard coded because u_16_integer size limit
...
void setup() {
  ...
  Timer0_Cfg = timerBegin(0, TIMER_PRESCALER_0, true);    // timerBegin(timer, prescaler, counUp)
  timerAttachInterrupt(Timer0_Cfg, &Timer0_ISR, true);  // True = Edge Triggered Timer & False = Level Triggered Timer
  timerAlarmWrite(Timer0_Cfg, TIMER_TICK_0, true);        // timerAlarmWrite(timer, TimerTick, autoReload)
  timerAlarmEnable(Timer0_Cfg);

  Timer1_Cfg = timerBegin(1, TIMER_PRESCALER_1, true);    // timerBegin(timer, prescaler, counUp)
