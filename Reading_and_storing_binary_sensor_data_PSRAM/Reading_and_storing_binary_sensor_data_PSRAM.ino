...
const uint8_t Sensor_size=5;
const uint16_t ADC_BUF_SIZE = 20480;  // index for the data in the ISR=======Max size try 9216
const uint16_t ADC_COPY_SIZE = 256;      // index for the data in the ISR =======Max size try 3072
...
void setup() {
  ...
  if (psramInit()) {
    ...
    ADC_BUFFER = (uint16_t**)ps_calloc(Sensor_size, sizeof(uint16_t*));
    for (int sensor = 0; sensor < Sensor_size; sensor++) {
      ADC_BUFFER[sensor] = (uint16_t*)ps_calloc(ADC_BUF_SIZE, sizeof(uint16_t));
    }

    //------------------PSRAM Free Space Check--------------------------------------
...
void loop() {
  ...
  if (start_flag == 1) {
    ...
    uint16_t ADC_buf_copy[Sensor_size][ADC_COPY_SIZE];  // variable to contain (ADC_copy_size x 2) bytes copy of the buffer
  ...
  if (ADC_flag == 1) {
    ...
    uint16_t ADC_buf_copy[Sensor_size][ADC_COPY_SIZE];
