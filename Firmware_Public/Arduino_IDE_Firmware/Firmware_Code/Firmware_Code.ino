...
const uint8_t Sensor_size=25;               // Analog SENSOR COUNT used for BUFFER SIZE
...
const uint8_t led_yellow = 45;              // To Indicate Battery Life or Voltage Level
const int Green_led = 21;                   // To control the built in GREEN LED
const int led_red = 00;                     // To Indicate the Data Reading of DAQ -- START or STOP--Blink 3 times
...
void loop() {
  ...
  if (ADC_BUFFER_Uncopied_flag == 1) {
    ...
    for (uint32_t i = 0; i < ((ADC_BUF_END_INDX - ADC_BUF_START_INDX + ADC_BUF_SIZE) % ADC_BUF_SIZE); i++) {
      ...
      Serial.print("--");
      Serial.print(ADC_buf_copy[14][i]);
      Serial.print("--");
      Serial.print(ADC_buf_copy[18][i]);
      Serial.print("--");
      Serial.print(ADC_buf_copy[22][i]);
      Serial.print("***");
      data_count++;
      ADC_BUF_START_INDX = (ADC_BUF_START_INDX + 1) % ADC_BUF_SIZE;
    }
