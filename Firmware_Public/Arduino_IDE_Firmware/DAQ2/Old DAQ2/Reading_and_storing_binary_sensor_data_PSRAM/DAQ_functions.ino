void read_analog_sensors(void){
  // if (DATA_READ_ENABLE) {
    if(DATA_READ_ENABLE && sensor_read_count<4){
      ...
      ADC_BUFFER[8][ADC_BUF_END_INDX] = analogRead(A8);  
      ADC_BUFFER[9][ADC_BUF_END_INDX] = analogRead(A13);
      ADC_BUFFER[10][ADC_BUF_END_INDX] = analogRead(A14);
      ADC_BUFFER[11][ADC_BUF_END_INDX] = analogRead(A15);  
      ADC_BUFFER[12][ADC_BUF_END_INDX] = analogRead(A16);
      ADC_BUFFER[13][ADC_BUF_END_INDX] = analogRead(A17);
